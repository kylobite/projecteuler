...
-- Problem 8
sort4max g = maximum $ get' g [] where
...
diagonal = filter (\x -> length x > 4) $ map (\x -> filter (/=1) x) $ concat $ loop horizontal [] where
    loop [] c = c
    loop xs c = loop (tail xs) ((map (\y -> (map (\x -> shift x y xs) [0..(length xs)-1])) [0..(length $ head xs)-1]):c)
    shift i j xs = let a = drop (i + j) (xs !! i)
                  in if a == []
                     then 1
                     else head a

diagonalr = filter (\x -> length x > 4) $ map (\x -> filter (/=1) x) $ concat $ loop (reverse horizontal) [] where
    loop [] c = c
    loop xs c = loop (tail xs) ((map (\y -> (map (\x -> shift x y xs) [0..(length xs)-1])) [0..(length $ head xs)-1]):c)
    shift i j xs = let a = drop (i + j) (xs !! i)
                  in if a == []
...
sort4max' g = maximum $ get' g [] where
    get' (_:_:_:_:[]) c = reverse c
    get' (x:xs) c       = get' xs ((x * (product $ (take 3 xs))):c)

p11 = maximum $ [(maximum $ map sort4max' horizontal),
                 (maximum $ map sort4max' vertical),
